...
updates_dir=/data/eunoiaos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have eunoiaos-VERSION-DATE-TYPE-* as format"
...
# Assume eunoiaos-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.eunoiaos.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.eunoiaos.updater/databases/updates.db" \
